...
prepped_dsa <- rsv_dectree$prep_dsa(settings, variation = "CrI")
...
res_dsa <- arrow::open_dataset(stringr::str_c(save_path, "/dsa")) |>
  collect() |>
  select(-vac_strategy)
...
res_dsa2 <- res_dsa |>
  ...
  mutate(
    ...
    param = case_when(
      ...
      param == "Mean_lifetime_prod_hours" ~
        "Mean hours of productivity for remainder of life",
      param == "Qaly_loss | local adverse event" ~
        "QALYs lost | Local adverse event",
      param == "Qaly_loss | qaly loss following hospitalization" ~
        "QALYs lost | Hospitalization",
      param == "Qaly_loss | severe adverse event" ~
        "QALYs lost | Severe adverse event",
      param == "Qaly_loss | systemic adverse event" ~
        "QALYs lost | Systemic adverse event",
      .default = param
    ),
    category = case_when(
      category == "Probs" ~ "Probabilities",
      category == "Base_case" ~ "base_case",
      category == "Vaccine_effectiveness" ~ "Vaccine effectiveness",
      .default = category
    )
  )

rsv_dectree$create_tornado(
  res_dsa2,
  settings,
  parameter = "icer",
  cutoff = 10,
  xlab = "Average cost-effectiveness ratio"
) +
  scale_fill_manual(values = rivernights_colors[c(6, 1)])


ggsave(
  stringr::str_c("VI. Tables and figures/", save_name, "_dsa_main.tiff"),
  scale = 1.5,
  height = 800,
  width = 1000,
  units = "px"
...
ggsave(
  stringr::str_c("VI. Tables and figures/", save_name, "_dsa_all.tiff"),
  scale = 1.5,
  height = 800,
  width = 1000,
  units = "px"
...
res_dsa_tbl <- res_dsa2 |>
  ...
  mutate(
    category = if_else(category == "base_case", "Base case", category),
    param = if_else(param == "base_case", "base case", param),
    perc_icer = (icer - base_case_icer) / base_case_icer,
    Variation = case_when(
      variation == "CrI" ~ "Based on 95% CrI",
      .default = variation
    )
  ) |>
  select(category, param, Variation, direction, icer, perc_icer) |>
  tidyr::pivot_wider(
    names_from = "direction",
    values_from = c("icer", "perc_icer")
  ) |>
  gt(groupname_col = "category", rowname_col = "param") |>
  fmt_currency(
    columns = starts_with("icer"),
    currency = "EUR",
    decimals = 0
  ) |>
  fmt_percent(
    columns = starts_with("perc_icer"),
    decimals = 0
  ) |>
  cols_merge(
    c("icer_Low value", "perc_icer_Low value"),
    pattern = "{1} ({2})"
  ) |>
  cols_merge(
    c("icer_High value", "perc_icer_High value"),
    pattern = "{1} ({2})"
  ) |>
  cols_label(
  ...
  tab_footnote(str_c(
    "Base case ICER is: €",
    vec_fmt_number(base_case_icer, decimals = 0),
    ". The percentage indicates the deviation from the base-case ICER."
  )) |>
  tab_footnote(
    "CrI: Credible Interval; ICER: Incremental Cost-Effectiveness Ratio, QALY: Quality Adjusted Life year, RSV: Respiratory Syncytial Virus"
  )
gtsave(
  res_dsa_tbl,
  filename = str_c("VI. Tables and figures/", save_name, "DSA_tabel.html")
)
